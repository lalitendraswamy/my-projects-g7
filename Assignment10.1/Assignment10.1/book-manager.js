...
class BookManager {
    ...
    toStringPrice(){
        var str = ``
        for (var n = this._books._first ; n ; n = n.next){
            str+= `Each Node : ${n.value.title}, Price: ${n.value.author} \n`
            // console.log(n)
        }
        return str;
    }
    ...
    getSortedByTitle() {
        var sortedll= this._books.sortedList((book1, book2) => book1.title - book2.title);
    ...
    getSortedByPrice() {
        var sortedll= this._books.sortedList((book1, book2) => book1.price - book2.price,false);
        return sortedll.toString();
    }
