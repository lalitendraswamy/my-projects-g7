...
var { isPrimeSync, findPrimesSync, findPrimes, findPrimesPromise,findPrimesWithGenerator } = require('../src/primes');
...
describe('findPrimesWithGenerator', () => {
    ...
    it('should handle the abort command correctly', function() {
        const gen = findPrimesWithGenerator(1, 10);
        let result = [];
        let next = gen.next();
        next = gen.next();
        next = gen.next();
        ...
        expect(next.value).to.deep.equal({ message: 'aborted', lastValueChecked: 5 });
    });
